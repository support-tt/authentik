...
-   Support for multiple blueprints in a single OCI artifact

    Instead of having to create a blueprint instance for each OCI artifact and each artifact only containg a single blueprint, an artifact can now contain multiple blueprints, which will be applied with the same context and in the order they are added. See more [here](../../developer-docs/blueprints/#storage---oci)
